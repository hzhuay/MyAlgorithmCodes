...
int main(){
    ...
    for(int i=1;i<=m;i++){
		scanf("%d",&opt);
        switch(opt){
	        case 0:scanf("%d",&x);if(find(1,x)==-1)insert(1,x);break;
	        case 1:scanf("%d",&x);if(find(1,x)==1)del(1,x);break;
	        case 2:scanf("%d",&x);int tmp=mx(1);while(x-->1){tmp=findpr(1,tmp-1);}printf("%d\n",tmp);break;
        }
    }
    return 0;
}
